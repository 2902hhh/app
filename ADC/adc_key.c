...
hi_void convert_to_voltage(hi_u32 data_len)
{
    ...
    for(i = 0; i< data_len; i++)
    {
        ...
        vlt_min = (voltage < vlt_max)? voltage : vlt_min;
    }
...
void my_gpio_adc_demo(void *arg)
{
    ...
    if(ret != HI_ERR_SUCCESS)
    {
        printf("[error] hi_gpio_set_dir1 ret:%d\r\n",ret);
        return;
    }
    ...
    while(1)
    {
        ...
        switch(get_key_event())
        {
            ...
            {
                printf("KEY_EVENT_S2 \r\n");
                hi_gpio_set_ouput_val(LED_TEST_GPIO,1);
                printf("LED OFF\n");
            }
            break;
            ...
            {
                printf("KEY_EVENT_S3 \r\n");
                hi_gpio_get_output_val(LED_TEST_GPIO,&led_val);
                if(led_val == 0)
                {
                    hi_gpio_set_ouput_val(LED_TEST_GPIO,1);
                    printf("LED OFF\n");
...
void key_demo(void)
{
    ...
    if(osThreadNew((osThreadFunc_t)my_gpio_adc_demo, NULL, &attr) == NULL)
    {
